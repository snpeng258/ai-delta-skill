...
横向从左到右是四层：**受到的冲击 → 要获得的能力 → 整理出的卡片 → 卡片怎么用**。第三层由 skill 自动写，第四层只有我能做。
...
flowchart LR
  ...
  subgraph L3["第三层 · 整理出的卡片 · skill 自动写"]
    direction TB
  ...
  subgraph L4["第四层 · 卡片怎么用"]
    direction TB
    LOAD["调用时加载<br/>先读标题 按需展开<br/>并提出关联候选"]
    AN{"我的分析<br/>跨多条 case 比较"}
    PH["phrasing.md<br/>只有我能写"]
    USE["开工时全抄 phrasing"]
    OUT["换成机器约束<br/>离开循环"]
  end
...
两条回流边是这张图的关键：`LOAD` 绕回词汇和拓扑，`USE` 绕回案例。前三层是一次性的推导，第四层是循环——**卡片被用掉才会长出新卡片**。
